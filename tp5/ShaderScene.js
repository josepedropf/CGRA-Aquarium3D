...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.texture3 = new CGFtexture(this, "textures/waterTex.jpg"); //new (remove comment later)
		this.texture4 = new CGFtexture(this, "textures/waterMap.jpg"); //new
		this.appearance.setTexture(this.texture3); //new
		//this.appearance.setTexture(this.texture4) //new
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/xanim.vert", "shaders/xanim.frag"),
			new CGFshader(this.gl, "shaders/texture1.vert", "shaders/grayscale.frag"),
			new CGFshader(this.gl, "shaders/water.vert", "shaders/water.frag"), //new
			new CGFshader(this.gl, "shaders/wateranim.vert", "shaders/wateranim.frag")
		];
		...
		this.testShaders[12].setUniformsValues({ uSampler2: 4 });
		//this.testShaders[13].setUniformsValues({ uSampler2: 3 });
		this.testShaders[13].setUniformsValues({ uSampler2: 4 });
	...
	update(t) {
		...
		if (this.selectedExampleShader == 6)
			...
			if (this.selectedExampleShader == 13)
			// Dividing the time by 100 "slows down" the variation (i.e. in 100 ms timeFactor increases 1 unit).
			// Doing the modulus (%) by 100 makes the timeFactor loop between 0 and 99
			// ( so the loop period of timeFactor is 100 times 100 ms = 10s ; the actual animation loop depends on how timeFactor is used in the shader )
			this.testShaders[13].setUniformsValues({ timeFactor: t / 100 % 100 });
	}
